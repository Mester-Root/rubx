...
class UserMethods:
    ...
    def send_code(
        ...
        return (GetData.api(version='5', tmp=UserMethods.tmp_generation(), method='sendCode', data=data, mode=Infos.citys[0], platform='web'))
    ...
    def upload_file(self, file: (str)) -> (dict):
        ...
        if (not 'http' in (file)):
            frequest, bytef = self.request_send_file(file).get('data'), open(file, 'rb').read()
