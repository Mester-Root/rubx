...
class UserMethods:
    ...
    def get_pending_object_owner(self, chat_id: str):
        '''
    ...
    def request_change_object_owner(self, chat_id: str):
        '''
        `self.request_change_object_owner('chat-guid')`
        '''
        
        return (GetData.api(version = self.api_version or '4', auth = self.auth,
                            method = 'requestChangeObjectOwner', data = {'object_guid': chat_id},
                            mode = self.city, platform = self.platform or 'android',
                            proxy = self.proxy))

    def chat_possession_transition(self, *args, **kwargs) -> (dict):
