...
class Metas(object):
    ...
    def __replacer(word: str) -> (str):
        pass
    
    @staticmethod
    def checker(text: str) -> (list):
        
        result, texts = [], text.replace('**', '', len(findall(r'\*\*(.*?)\*\*', text)) + 1).replace('__', '', len(findall(r'(\_\_(.*?)\_\_)', text)) + 1).replace('``', '', len(findall(r'\`\`(.*?)\`\`', text)) + 1)
...
class UserMethods:
    ...
    def send_message(
        ...
        parse_mode          :   (str)   =   None
            ...
            if (parse_mode or text.startswith('__') or text.startswith('**') or text.startswith('``') or len(findall(r'<(.*?)>', text)) + 1 >= 2):
                ...
                if parse_mode.lower() == 'markdown' or any([*text].count(meta) >= 2 for meta in ('__', '**', '``')):
    ...
    def edit_message(
        ...
        if (parse_mode or text.startswith('__') or text.startswith('**') or text.startswith('``') or len(findall(r'<(.*?)>', text)) + 1 >= 2):
            if any([*text].count(meta) >= 2 for meta in ('__', '**', '``')):
